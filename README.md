...
<head>
    ...
    <title>現代戰爭：隨機戰域 (Modern Warfare: Rogue Map)</title>
    <style>
        ...
        .upgrade-card { background: linear-gradient(180deg, rgba(20,20,20,0.9) 0%, rgba(40,40,40,0.9) 100%); border: 2px solid #555; border-radius: 12px; padding: 30px; cursor: pointer; transition: all 0.2s; text-align: left; position: relative; overflow: hidden; }
...
<body>
    ...
    <div id="game-ui">
        ...
        <div id="weapon-panel" class="hud-panel">
            <div id="weapon-name">制式步槍</div>
            <div id="weapon-desc">標準配發武器</div>
        ...
        <div id="skills-panel">
            ...
            <div class="skill-box ready" id="skill-q">
                ...
                <span class="skill-icon" id="skill-icon-q">🚀</span>
        ...
        <div id="elite-notification" class="notification">
            <h2 style="color:#9333ea; text-shadow: 0 0 15px #9333ea; font-size: 42px;">⚠️ ELITE DETECTED ⚠️</h2>
        ...
        <div id="char-select" class="hud-panel modal-screen">
            ...
            <div class="char-grid">
                <div class="char-card infantry" onclick="selectClass('infantry')">
                    <span class="char-icon" style="color:#3b82f6">🛡️</span><span class="char-title">步兵</span><p class="char-skill">技能: <span class="skill-highlight">精準導彈</span></p>
                </div>
                <div class="char-card engineer" onclick="selectClass('engineer')">
                    <span class="char-icon" style="color:#f97316">⚙️</span><span class="char-title">工程兵</span><p class="char-skill">技能: <span class="skill-highlight">周圍爆破</span></p>
                </div>
                <div class="char-card medic" onclick="selectClass('medic')">
                    <span class="char-icon" style="color:#10b981">✚</span><span class="char-title">醫療兵</span><p class="char-skill">技能: <span class="skill-highlight">戰場急救</span></p>
                    <p style="font-size:0.8em; color:#00ff00; margin-top:5px;">(被動: 移動速度 +5%)</p>
                </div>
                <div class="char-card sniper" onclick="selectClass('sniper')">
                    <span class="char-icon" style="color:#94a3b8">🎯</span><span class="char-title">狙擊手</span><p class="char-skill">技能: <span class="skill-highlight">一擊斃殺</span></p>
                </div>
            </div>
        </div>
        ...
        <div id="common-upgrade-modal" class="hud-panel modal-screen" style="display:none;">
            <h1 id="modal-title" style="color:#ffff00; text-shadow: 0 0 10px red;">UPGRADE AVAILABLE</h1>
            <p id="modal-desc">Combat data analysis complete. Choose an upgrade module.</p>
            <div class="upgrade-grid" id="upgrade-options-container">
                <!-- Dynamic Content -->
            </div>
        </div>
    ...
    <script>
        ...
        // --- 資料設定 ---
        const classes = {
            infantry: { name: "步兵", skill: "精準導彈", color: 0x3b82f6, cd: 300, icon: '🚀', missileCount: 1, missileMode: 'normal' },
            engineer: { name: "工程兵", skill: "周圍爆破", color: 0xf97316, cd: 480, icon: '💥', blastRadius: 8, blastMode: 'normal' },
            medic: { name: "醫療兵", skill: "急救 (25HP)", color: 0x10b981, cd: 1800, icon: '✚', healAmount: 25, healMode: 'normal', speedBonus: 1.05 },
            sniper: { name: "狙擊手", skill: "一擊斃殺", color: 0x94a3b8, cd: 600, icon: '🎯', shotCount: 1, shotMode: 'normal' }
        };

        const weapons = [
            { name: "制式步槍", desc: "平衡的標準武器", fireRate: 10, damage: 35, speed: 0.8, color: 0xffff00, count: 1, spread: 0.02, life: 100, size: 0.15, pitch: 1.0 },
            { name: "輕型衝鋒槍", desc: "高射速，低傷害", fireRate: 5, damage: 18, speed: 0.7, color: 0xffcc00, count: 1, spread: 0.15, life: 70, size: 0.12, pitch: 1.5 },
            { name: "雙持麥格農", desc: "雙倍火力", fireRate: 20, damage: 45, speed: 0.9, color: 0xffffff, count: 2, spread: 0.1, life: 90, size: 0.18, pitch: 0.8 },
            { name: "戰術散彈槍", desc: "近距離打擊", fireRate: 35, damage: 25, speed: 0.6, color: 0xffaa00, count: 5, spread: 0.3, life: 40, size: 0.12, pitch: 0.6 },
            { name: "重型機槍", desc: "持續火力壓制", fireRate: 8, damage: 45, speed: 0.85, color: 0xff8800, count: 1, spread: 0.05, life: 110, size: 0.2, pitch: 0.7 },
            { name: "精準射手步槍", desc: "高傷害", fireRate: 25, damage: 95, speed: 1.5, color: 0x00ff00, count: 1, spread: 0.0, life: 150, size: 0.2, pitch: 2.0 },
            { name: "火焰噴射器", desc: "燃燒一切", fireRate: 2, damage: 10, speed: 0.45, color: 0xff4400, count: 1, spread: 0.15, life: 35, size: 0.3, isFlame: true, pitch: 3.0 },
            { name: "榴彈發射器", desc: "範圍爆炸", fireRate: 50, damage: 0, areaDmg: 150, areaRadius: 4, speed: 0.6, color: 0x444444, count: 1, spread: 0.05, life: 60, size: 0.4, explosive: true, pitch: 0.5 },
            { name: "脈衝步槍", desc: "能量武器", fireRate: 6, damage: 35, speed: 1.2, color: 0x00ffff, count: 1, spread: 0.01, life: 120, size: 0.15, pitch: 1.8 },
            { name: "加特林機槍", desc: "極致射速", fireRate: 3, damage: 22, speed: 0.9, color: 0xffffaa, count: 1, spread: 0.12, life: 90, size: 0.12, pitch: 1.2 },
            { name: "火箭發射器", desc: "重型毀滅", fireRate: 70, damage: 0, areaDmg: 350, areaRadius: 6, speed: 0.5, color: 0x888888, count: 1, spread: 0, life: 80, size: 0.5, explosive: true, pitch: 0.4 },
            { name: "雷射加農砲", desc: "光速穿透", fireRate: 40, damage: 250, speed: 3.0, color: 0xff00ff, count: 1, spread: 0, life: 60, size: 0.1, pitch: 2.5, penetrate: true },
            { name: "虛空毀滅者", desc: "終極兵器", fireRate: 15, damage: 60, areaDmg: 100, areaRadius: 3, speed: 0.7, color: 0xaa00ff, count: 5, spread: 0.4, life: 100, size: 0.3, explosive: true, pitch: 0.3 },
            { name: "生化腐蝕砲", desc: "高速酸液噴射", fireRate: 4, damage: 15, speed: 1.1, color: 0x00ff00, count: 1, spread: 0.1, life: 60, size: 0.25, pitch: 1.5 },
            { name: "智能追蹤導彈", desc: "自動鎖定敵人", fireRate: 25, damage: 45, speed: 0.9, color: 0x00aaff, count: 3, spread: 0.5, life: 150, size: 0.2, homing: true, pitch: 1.0 },
            { name: "震盪衝擊波", desc: "寬幅穿透擊退", fireRate: 45, damage: 80, speed: 0.5, color: 0xffffff, count: 1, spread: 0, life: 80, size: 1.5, penetrate: true, knockback: 2.0, pitch: 0.2 },
            { name: "雙子星雷射", desc: "雙聯平行毀滅", fireRate: 8, damage: 40, speed: 1.8, color: 0xff0055, count: 2, spread: 0.05, life: 100, size: 0.15, pitch: 2.2, parallel: true },
            { name: "終焉奇點", desc: "毀滅性黑洞", fireRate: 120, damage: 10, areaDmg: 800, areaRadius: 12, speed: 0.3, color: 0x000000, count: 1, spread: 0, life: 150, size: 0.8, explosive: true, pitch: 0.1 },
            { name: "雙持烏茲", desc: "雙重彈幕", fireRate: 3, damage: 10, speed: 0.7, color: 0xffcc00, count: 2, spread: 0.25, life: 60, size: 0.12, pitch: 1.5, dual: true },
            { name: "雙持削短型散彈", desc: "雙管齊下", fireRate: 50, damage: 18, speed: 0.6, color: 0xdd8800, count: 8, spread: 0.35, life: 30, size: 0.12, pitch: 0.6, dual: true },
            { name: "雙持熱熔手槍", desc: "穿透高熱", fireRate: 15, damage: 45, speed: 0.6, color: 0xff4400, count: 2, spread: 0.05, life: 80, size: 0.2, pitch: 1.2, dual: true, penetrate: true },
            { name: "雙持針刺者", desc: "雙重追蹤", fireRate: 5, damage: 12, speed: 0.8, color: 0xff00ff, count: 2, spread: 0.2, life: 100, size: 0.15, pitch: 1.8, dual: true, homing: true },
            { name: "雙持重砲", desc: "雙重爆破", fireRate: 30, damage: 0, areaDmg: 120, areaRadius: 4, speed: 0.5, color: 0x444444, count: 2, spread: 0.1, life: 80, size: 0.3, pitch: 0.5, dual: true, explosive: true }
        ];
        ...
        const config = {
            ...
            dropChance: 0.1, eliteChance: 0.15, dropHealAmount: 20,
        ...
        let gameState = {
            isPlaying: false, playerClass: null, score: 0, wave: 1, hp: 100,
            lastShot: 0, dashCooldown: 0, skillCooldown: 0, dashFrames: 0, frameCount: 0,
            weaponIndex: 0, weaponDamageMult: 1.0, 
            sniperBuff: false, sniperShotCount: 0, invulnerable: 0,
        ...
        function init() {
            ...
            createBoundaries(); // createObstacles called in startGame
            ...
            ui.muteBtn.addEventListener('click', () => { const isMuted = sfx.toggleMute(); ui.muteBtn.innerText = isMuted ? '🔇' : '🔊'; });
        ...
        function startGame() {
            gameState.isPlaying = true; isPaused = false; gameState.score = 0; gameState.hp = 100; gameState.wave = 1; 
            gameState.weaponIndex = 0; gameState.skillCooldown = 0; gameState.dashCooldown = 0; 
            gameState.weaponDamageMult = 1.0;
            gameState.sniperBuff = false; gameState.invulnerable = 0;
            ...
            classes.sniper.shotCount = 1; classes.sniper.shotMode = 'normal';
        ...
        function showSkillUpgrade() {
            isPaused = true;
            ui.upgradeModal.style.display = 'flex';
            ui.modalTitle.innerText = "SKILL EVOLUTION";
            ui.modalDesc.innerText = "Class proficiency improved. Select tactical upgrade.";
            
            const cls = classes[gameState.playerClass];
            let optionA = {}, optionB = {};

            if (gameState.playerClass === 'infantry') {
                optionA = { title: "增援打擊", desc: "導彈數量 +1", type: "QUANTITY", action: () => { cls.missileCount++; } };
                optionB = { title: "地毯式轟炸", desc: "直線轟炸模式", type: "QUALITY", action: () => { cls.missileMode = 'carpet'; } };
            } else if (gameState.playerClass === 'engineer') {
                optionA = { title: "系統超頻", desc: "冷卻 -25%", type: "QUANTITY", action: () => { cls.cd = Math.floor(cls.cd * 0.75); } };
                optionB = { title: "電漿新星", desc: "爆破半徑加倍", type: "QUALITY", action: () => { cls.blastMode = 'nova'; cls.blastRadius = 12; } };
            } else if (gameState.playerClass === 'medic') {
                optionA = { title: "高效治療", desc: "補血量 +100%", type: "QUANTITY", action: () => { cls.healAmount = 50; } };
                optionB = { title: "奈米護盾", desc: "治療時無敵 3 秒", type: "QUALITY", action: () => { cls.healMode = 'shield'; } };
            } else if (gameState.playerClass === 'sniper') {
                optionA = { title: "連環殺手", desc: "技能次數 +1", type: "QUANTITY", action: () => { cls.shotCount++; } };
                optionB = { title: "爆破彈頭", desc: "技能附加爆炸", type: "QUALITY", action: () => { cls.shotMode = 'explosive'; } };
            }
            renderUpgradeCards(optionA, optionB);
        }

        function showWeaponUpgrade() {
            isPaused = true;
            ui.upgradeModal.style.display = 'flex';
            ui.modalTitle.innerText = "WEAPON MASTERY";
            ui.modalDesc.innerText = "Armory access granted. Improve current gear or requisition new equipment.";

            const currentDmg = Math.round(gameState.weaponDamageMult * 100);
            
            const optionA = { 
                title: "極限超頻", 
                desc: `保留當前武器，攻擊力 +50% (目前: ${currentDmg}%)`, 
                type: "ENHANCE", 
                action: () => { 
                    gameState.weaponDamageMult += 0.5; 
                    updateWeaponUI();
                } 
            };
            ...
            const optionB = { 
                title: "軍火重配", 
                desc: "隨機獲得一把新武器 (攻擊力重置為 110%)", 
                type: "REROLL", 
                action: () => { 
                    let newIndex; 
                    do { newIndex = Math.floor(Math.random() * weapons.length); } while (newIndex === gameState.weaponIndex && weapons.length > 1);
                    gameState.weaponIndex = newIndex;
                    gameState.weaponDamageMult = 1.1; // 換新槍給一點點獎勵
                    updateWeaponUI();
                } 
            };
            renderUpgradeCards(optionA, optionB);
        ...
        function renderUpgradeCards(optA, optB) {
            ui.upgradeContainer.innerHTML = `
                <div class="upgrade-card" onclick="selectUpgrade('A')">
                    <span class="upgrade-type">${optA.type}</span>
                    <span class="upgrade-title" style="color:#00ffaa">${optA.title}</span>
                    <p class="upgrade-desc">${optA.desc}</p>
                </div>
                <div class="upgrade-card" onclick="selectUpgrade('B')">
                    <span class="upgrade-type">${optB.type}</span>
                    <span class="upgrade-title" style="color:#ffaa00">${optB.title}</span>
                    <p class="upgrade-desc">${optB.desc}</p>
                </div>
            `;
            window.upgradeActions = { A: optA.action, B: optB.action };
        }

        window.selectUpgrade = function(choice) {
            if (window.upgradeActions && window.upgradeActions[choice]) {
                window.upgradeActions[choice]();
                sfx.playUpgrade();
            }
            ui.upgradeModal.style.display = 'none';
            isPaused = false;
        };
        ...
        function updatePlayer() {
            ...
            if (gameState.dashFrames > 0) {
                ...
                gameState.dashFrames--;
                if (gameState.frameCount % 3 === 0) createParticle(player.position, cls.color, 0.5);
            } else if (keys.shift && gameState.dashCooldown <= 0) {
                gameState.dashFrames = config.dashDuration;
                gameState.dashCooldown = 120;
                ui.skillDash.classList.remove('ready');
                ui.skillDash.classList.add('cooldown');
                shakeCamera(0.2);
            }
            if (keys.q && gameState.skillCooldown <= 0) {
                activateClassSkill();
                gameState.skillCooldown = cls.cd;
                ui.skillBoxQ.classList.remove('ready');
                ui.skillBoxQ.classList.add('cooldown');
            }
            ...
            if (dx !== 0 || dz !== 0) {
                ...
                const playerBox = new THREE.Box3().setFromCenterAndSize(
                    new THREE.Vector3(nextX, 0.5, nextZ), new THREE.Vector3(0.8, 1, 0.8)
                );
                ...
                if (!collision) {
                    player.position.x = Math.max(-config.boundaryLimit, Math.min(config.boundaryLimit, nextX));
                    player.position.z = Math.max(-config.boundaryLimit, Math.min(config.boundaryLimit, nextZ));
                }
            }
            ...
            for (let i = 0; i < intersects.length; i++) {
                if (intersects[i].object.name === "ground") {
                    const lookTarget = intersects[i].point;
        ...
        function shoot() {
            ...
            if (isLethal) { 
                gameState.sniperShotCount--;
                if (gameState.sniperShotCount <= 0) { gameState.sniperBuff = false; ui.skillBoxQ.classList.remove('skill-active'); }
                sfx.playShoot(0.5); shakeCamera(1.0); 
            } else { 
                sfx.playShoot(weapon.pitch); if(weapon.explosive || weapon.damage > 50) shakeCamera(0.2); 
            }
            ...
            for(let i=0; i<count; i++) {
                ...
                let areaDmg = weapon.areaDmg * gameState.weaponDamageMult;
                ...
                bullets.push({
                    ...
                    life: weapon.life, damage: finalDmg,
                    explosive: explosive, areaDmg: areaDmg, areaRadius: areaRadius,
        ...
        function createObstacles() {
            ...
            const count = 12 + Math.floor(Math.random() * 5); // 12-16 個障礙物
            for(let i=0; i<count; i++) {
                let x, z, valid = false;
                // 嘗試生成不靠近原點的座標
                while(!valid) {
                    ...
                    if (Math.sqrt(x*x + z*z) > 10) valid = true; // 遠離出生點 10 單位
                }
                ...
                mesh.rotation.y = Math.random() * Math.PI; // 隨機旋轉
                mesh.castShadow = true; mesh.receiveShadow = true;
                scene.add(mesh);
                mesh.geometry.computeBoundingBox();
                // 更新 bounding box 以匹配旋轉 (簡化：這裡使用 OBB 概念太複雜，我們用稍大的 AABB 近似，或者不旋轉碰撞體只旋轉 mesh)
                // 為求簡單物理，這裡不旋轉物理判定盒，只旋轉視覺 Mesh 會導致穿模，所以還原旋轉或使用圓形碰撞
                // 修正：不旋轉 Box 以保持 AABB 邏輯簡單
                mesh.rotation.y = 0; 
        ...
        // --- 生成邏輯 (更新) ---
        function spawnEnemy() { 
            let x,z; const side=Math.floor(Math.random()*4); const limit=45; const offset=(Math.random()-0.5)*2*limit; 
            switch(side){case 0:x=offset;z=-limit;break;case 1:x=limit;z=offset;break;case 2:x=offset;z=limit;break;case 3:x=-limit;z=offset;break;} 
            
            const timeInSeconds = gameState.frameCount / 60;
        ...
        function updateEnemies() { 
            ...
            for(let i=0;i<enemies.length;i++){ 
                ...
                if(e.type==='ranged'){ 
                    if(distToPlayer<12)shouldMove=false; 
                    if(gameState.frameCount-e.lastShot>120){ shootEnemyBullet(e); e.lastShot=gameState.frameCount; } 
                } 
                ...
                if(shouldMove){ 
                    ...
                    for (let j = 0; j < enemies.length; j++) {
                        if (i === j) continue;
                        const dist = e.mesh.position.distanceTo(enemies[j].mesh.position);
                        const safeDist = (e.isElite ? 2.0 : 1.2);
                        if (dist < safeDist) {
                            const push = new THREE.Vector3().subVectors(e.mesh.position, enemies[j].mesh.position).normalize();
                    ...
                    if (distToPlayer < 1.5) {
                        const push = new THREE.Vector3().subVectors(e.mesh.position, player.position).normalize();
                        push.multiplyScalar(2.0);
                        separation.add(push);
                        count++;
                    }
                    if (count > 0) {
                        separation.divideScalar(count).normalize().multiplyScalar(0.05);
                        mv.add(separation);
                    }
                ...
                if(e.type!=='ranged'&&distToPlayer<(e.isElite?1.8:1.2)){ if(gameState.invulnerable<=0){ gameState.hp-=e.isElite?1.5:0.5; updateUI(); shakeCamera(0.1); if(gameState.hp<=0)gameOver(); } } 
            } 
        }
        function shootEnemyBullet(enemy) { const bulletGeo=new THREE.SphereGeometry(0.2,8,8); const bulletMat=new THREE.MeshBasicMaterial({color:0xff0000}); const bullet=new THREE.Mesh(bulletGeo,bulletMat); bullet.position.copy(enemy.mesh.position); bullet.position.y=0.6; const direction=new THREE.Vector3().subVectors(player.position,enemy.mesh.position).normalize(); bullet.position.add(direction.clone().multiplyScalar(0.8)); scene.add(bullet); enemyBullets.push({mesh:bullet,velocity:direction.multiplyScalar(0.35),life:100,damage:10}); sfx.playEnemyShoot(); }
        function activateClassSkill() { const cls=classes[gameState.playerClass]; if(gameState.playerClass==='infantry'){ callAirstrike(cls.missileCount,cls.missileMode); } else if(gameState.playerClass==='engineer'){ createExplosion(player.position,0xf97316,12); applyAreaDamage(player.position,cls.blastRadius,1000); const ringGeo=new THREE.RingGeometry(0.5,cls.blastRadius,32); const ringMat=new THREE.MeshBasicMaterial({color:cls.blastMode==='nova'?0x00ffff:0xf97316,side:THREE.DoubleSide,transparent:true,opacity:0.8}); const ring=new THREE.Mesh(ringGeo,ringMat); ring.rotation.x=-Math.PI/2; ring.position.copy(player.position); ring.position.y=0.1; scene.add(ring); const expand=()=>{ring.scale.multiplyScalar(1.1);ring.material.opacity-=0.05;if(ring.material.opacity>0)requestAnimationFrame(expand);else scene.remove(ring);}; expand(); if(cls.blastMode==='nova')createDebris(player.position,0x00ffff); } else if(gameState.playerClass==='medic'){ gameState.hp=Math.min(100,gameState.hp+cls.healAmount); createFloatingText(player.position,`+${cls.healAmount} HP`,'#00ff00'); updateUI(); sfx.playHeal(); createParticle(player.position,0x00ff00,20); if(cls.healMode==='shield'){gameState.invulnerable=180;createFloatingText(player.position,`SHIELDED`,'#ffffff');} } else if(gameState.playerClass==='sniper'){ gameState.sniperBuff=true; gameState.sniperShotCount=cls.shotCount; ui.skillBoxQ.classList.add('skill-active'); createFloatingText(player.position,"LETHAL READY x"+cls.shotCount,"#ffffff"); sfx.playSniperCharge(); } }
        function callAirstrike(count,mode) { raycaster.setFromCamera(mouse,camera); const ints=raycaster.intersectObject(scene.children.find(c=>c.name==="ground")); if(ints.length>0){ const t=ints[0].point; if(mode==='carpet'){for(let i=-1;i<=1;i++){const offsetT=t.clone();offsetT.x+=i*6;createStrikeIndicator(offsetT,i*10);}} else {for(let i=0;i<count;i++){const offsetT=t.clone();if(i>0){offsetT.x+=(Math.random()-0.5)*10;offsetT.z+=(Math.random()-0.5)*10;}createStrikeIndicator(offsetT,i*10);}} } }
